...
public class CityLotBuilder {
    ...
    public void buildLots(SimCraft.RegionBuildState buildState) {
        buildState.setBuildState(SimCraft.BuildState.LOTS);
        buildState.setProgress(0);

        double maxProgress = this.region.getLots().values().size();
        int i = 0;
        for (LotData lot : this.region.getLots().values()) {
            this.buildLot(lot);
